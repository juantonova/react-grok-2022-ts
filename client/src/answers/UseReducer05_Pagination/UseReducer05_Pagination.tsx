...
function UseReducer05_Pagination(): JSX.Element {
  ...
  const setArrPages = (event: React.ChangeEvent<HTMLSelectElement>): void => {
    fetch(`${process.env.REACT_APP_SERVER_URL}/cities/count`, {
      method: 'POST',
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_SERVER_URL}/pages/1?limit=${state.limit}`)
  ...
  const choosePage = (page: number): void => {
    fetch(
      `${process.env.REACT_APP_SERVER_URL}/pages/${page}?limit=${state.limit}`
    )
      .then((response) => response.json())
      .then((data) => {
        dispatch({ type: 'GET_CITIES', payload: data });
